...
// entering with SPACE/L, the per-level Beat-Kit routes to actually beat 1-1…2-3,
// and the clear-overlay CONTINUE back to the hub — then the TUTORIAL launched
...
//   • save/unlock progression: fresh save → 1-1…2-3 unlocked/cleared IN ORDER
//     (the bolt-buddies-save-v1 `unlocked` count advances a step per clear);
//     cores array + a ux-v1 best-time RECORD persist for each cleared chamber.
...
//   context: title → 6 levels → tutorial, in a single session, exercising real
...
const LEVELS = ["1-1", "1-2", "1-3", "2-1", "2-2", "2-3"]; // registry idx 0..5
const ROLES = { G: 0, H: 1, P: 0, T: 1 }; // P1 takes the first pedestal (runner "A")
...
const hubInfo = (page) => page.evaluate(() => {
  ...
  return {
    sel: h.sel,
    count: h.hubCount,
    nodes: h.nodes.map((n) => ({ idx: n.idx, unlocked: n.unlocked })),
  };
});
...
async function runRouteToComplete(page, bb, i) {
  ...
  if (!st || st.complete !== true) throw new Error(`${id} route ran but level not complete`);
}
...
async function playLevel(page, bb, i) {
  ...
  while (attempt < MAX_LEVEL_ATTEMPTS) {
    ...
    try {
      ...
      await bb.releaseAll().catch(() => {});
      if (!(await active(page, "Hub"))) await abortToHub(page, bb).catch(() => {});
  ...
  // unlock progression is visible on the very hub we navigate: the next node is
  // now unlocked (and this one shows completed).
...
// ---------------------------------------------------------------------------
// one full campaign: title → NEW GAME → skip → 6 levels → tutorial
// ---------------------------------------------------------------------------
async function runCampaign(page, bb, no, errCountAtStart, pageErrors) {
  ...
  assert((await readSave(page)) === null, "fresh save (no bolt-buddies-save-v1)");
  ...
  process.stdout.write(`\n  ▶ tutorial (from menu) ...`);
  const tut = await runTutorialFromMenu(page, bb);
  ...
  const clean = flakes === 0 && pageErrsThis === 0 && wdMax === 0 && slMax === 0;
  ...
  process.stdout.write(
    `  campaign ${no}: 6/6 levels beat + tutorial · flake-retries ${flakes} · ` +
...
async function main() {
  ...
  for (const c of campaigns) {
    ...
    console.log(
      ...
      `6/6 levels + tutorial, flake-retries ${c.flakes}, page errors ${c.pageErrsThis}, ` +
  ...
  const ok = forcedCampaigns ? campaigns.every((c) => !c.failed && c.clean) : twoClean;
  console.log(`\n  RESULT: ${ok ? `PASS — ${TARGET_CLEAN} consecutive CLEAN campaigns` : (fatal ? `FAIL — ${fatal}` : "FAIL — did not reach two consecutive clean campaigns")}`);
