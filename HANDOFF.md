# HANDOFF — Bolt Buddies (co-op puzzle platformer)

**Paste this entire document as the first message in a Claude Code web session.** It is fully self-contained — no local files are needed.

## What this is

I (Jonathan) spec'd out a 2-player couch co-op puzzle platformer in a previous local Claude Code session. All product decisions are final (they were made through a structured Q&A). The complete game design document is embedded below.

**Your job in this session: build the game.** Follow the Build Roadmap in the design doc, starting at step 1 (engine core) and step 2 (vertical slice of Level 1-1). Work autonomously through the roadmap — the design decisions are already made, don't re-litigate them.

## Current state

- **No code exists yet.** This is a greenfield build.
- The only prior artifact is the design doc below (it lived at `bolt-buddies/GAME_DESIGN.md` on my laptop; recreate it as `GAME_DESIGN.md` at the repo root so it ships with the code).

## Practical constraints

- **Tech stack (decided):** Phaser 3, HTML5 canvas, plain JS or TS (your call), targeting 60fps. Keep it deployable as a static site.
- **Deploy target:** Vercel. Structure the project so `vercel` / a static build works out of the box.
- **Git identity:** all commits must be authored as `aliceagent <258758174+aliceagent@users.noreply.github.com>` — never a levana.finance email.
- **Verification:** playtest as you go — run the dev server and actually drive both characters (P1: A/D/W + E, P2: arrows + L) to confirm each mechanic before moving on.
- **Definition of done for this session:** roadmap steps 1–3 (engine core, Level 1-1 vertical slice, then World 1 complete with hub map, data-cores, and localStorage save). If you get further, continue into World 2.

## Open questions you may decide yourself during the build

- Final title (*Bolt Buddies*, *Beep & Boop*, *Rescue Bolt!* — pick one, easy to rename later)
- Camera: one shared view with soft zoom vs. split-screen when players separate (start with shared view + zoom; only add split if levels demand it)
- Sound: chiptune vs. synthwave (either; SPARK "voice" as text-blips, no TTS)
- Solo mode is explicitly **post-launch** — do not build it now.

---

# GAME_DESIGN.md (embedded, verbatim)
